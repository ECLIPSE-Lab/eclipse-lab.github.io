---
pagetitle: "People | ECLIPSE Lab"
toc: false
listing:
  - id: research-staff
    contents: "people/staff/*.qmd"
    type: grid
    image-height: 225px
    grid-columns: 3
    fields: [image, title, subtitle, interests]
    grid-item-align: center
    sort: sortby
    field-display-names: 
      started: "Started"
  - id: msc_students
    contents: "people/msc/*.qmd"
    type: grid
    image-height: 225px
    grid-columns: 3
    sort: sortby
    fields: [image, title, subtitle, started]
    field-display-names: 
      started: "Started"
    grid-item-align: center
  - id: admin-assistants
    contents: "people/admins/*.qmd"
    type: grid
    image-height: 225px
    grid-columns: 3
    sort: sortby
    fields: [image, title, subtitle, started]
    grid-item-align: center
  - id: alumni
    contents: "people/alumni/*.qmd"
    type: table
    sort: sortby
    fields: [title, subtitle, started, ended]
    sort-ui: [title, subtitle, started, ended]
    filter-ui: [title, subtitle, started, ended]
    field-display-names: 
      title: "Name"
      subtitle: "Role"
      started: "Started"
      ended: "Ended"
description: "Meet the ECLIPSE Lab team at FAU Erlangen-Nürnberg — researchers, PhD students, and alumni working on computational microscopy and electron imaging."

---

```{=html}
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    <li class="breadcrumb-item"><a href="index.html">Home</a></li>
    <li class="breadcrumb-item active" aria-current="page">People</li>
  </ol>
</nav>
```


## Jump to

[Research Staff](#research-staff) · [MSc Students](#msc-students) · [Research Assistants](#research-assistants) · [BSc Students](#bsc-students) · [Admin Assistants](#admin-assistants) · [Alumni](#alumni)

---

## Research Staff {#research-staff}

:::{#research-staff}
:::

## MSc Students {#msc-students}

:::{#msc_students}
:::

## Research Assistants {#research-assistants}

Currently no research assistants are listed.

## BSc Students {#bsc-students}

Currently no BSc students are listed.

## Administrative Assistants {#admin-assistants}
:::{#admin-assistants}
:::

## Alumni {#alumni}

:::{#alumni}
:::
